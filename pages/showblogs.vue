    ...
    <v-row>
      <v-col v-for="post in posts" :key="post.id" cols="12" md="4">
        <v-card class="mt-6">
          ...
          <v-card-actions>
            <v-btn style="background-color: green; color: white;" :to="{ name: 'id', params: { id: post.id } }"
              >Read</v-btn
            >
            <v-spacer></v-spacer>
            <v-btn
            ...
            >
            <v-spacer></v-spacer>
            <v-btn style="background-color: red; color: white;" @click="deletePost(post.id)">Delete</v-btn>
          </v-card-actions>
        </v-card>
      </v-col>
    </v-row>
